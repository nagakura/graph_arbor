...
(function($){
    ...
    var that = {
      ...
      redraw:function(){
        ...
        sys.eachNode(function(node, pt){
          var w = Math.max(40, 40+gfx.textWidth(node.name) )
          if (node.data.alpha===0) return
          if (node.data.shape=='dot'){
            gfx.oval(pt.x-w/2, pt.y-w/2, w, w, {fill:node.data.color, alpha:node.data.alpha})
            gfx.text(node.data.title, pt.x, pt.y+7, {color:"white", align:"center", font:"Arial", size:12})
            gfx.text(node.data.title, pt.x, pt.y+7, {color:"white", align:"center", font:"Arial", size:12})
          }else{
            gfx.rect(pt.x-w/2, pt.y-8, w, 20, 4, {fill:node.data.color, alpha:node.data.alpha})
            gfx.text(node.data.title, pt.x, pt.y+9, {color:"black", align:"center", font:"Arial", size:12})
            gfx.text(node.data.title, pt.x, pt.y+9, {color:"black", align:"center", font:"Arial", size:12})
          }
        })
        that._drawVignette()
  ...
  */
  ...
  $(document).ready(function(){
    ...
    var theUI = {
      nodes:{"arbor.js":{title:"メンバー2013", color:"red", shape:"dot", alpha:1}, 
      
             demos:{title:"shokai", color:CLR.branch, shape:"dot", alpha:1}, 
             halfviz:{title:"RocketIO",color:CLR.demo, alpha:0, link:'/halfviz'},
             atlas:{title:"GoldFish",color:CLR.demo, alpha:0, link:'/atlas'},
             echolalia:{title:"降臨",color:CLR.demo, alpha:0, link:'/echolalia'},

             docs:{title:"geta6", color:CLR.branch, shape:"dot", alpha:1}, 
             reference:{title:"〓サーバー",color:CLR.doc, alpha:0, link:'#reference'},
             introduction:{title:"ふぁぼらせろ",color:CLR.doc, alpha:0, link:'#introduction'},

             code:{title:"marutaru", color:CLR.branch, shape:"dot", alpha:1},
             github:{title:"web-it",color:CLR.code, alpha:0, link:'https://github.com/samizdatco/arbor'},
             ".zip":{title:"soya",color:CLR.code, alpha:0, link:'/js/dist/arbor-v0.92.zip'},
             ".tar.gz":{title:"片手インタフェース",color:CLR.code, alpha:0, link:'/js/dist/arbor-v0.92.tar.gz'}
            },
      edges:{
        "arbor.js":{
          demos:{length:.8},
